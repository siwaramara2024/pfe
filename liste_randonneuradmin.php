...
$sql = "SELECT * FROM Randonneur";
if (isset($_GET['query'])) {
    $search_query = $_GET['query'];
    // Vous pouvez utiliser la fonction de recherche LIKE pour rechercher dans le titre1 et l'id
    $sql = "SELECT * FROM Randonneur WHERE CIN LIKE '%$search_query%'OR Nom = '$search_query' OR Prenom = '$search_query'
    OR Email = '$search_query'OR Num_tel = '$search_query' OR libelleV = '$search_query'";
}
$result = $conn->query($sql);
?>
...
<head>
    ...
    <title>Liste des Randonneurs</title>
...
<hr class="dropdown-divider">
  ...
  <li class="nav-item">
    ...
    <li class="nav-item">
      <a class="nav-link "  href="liste_randonneuradmin.php">
        <i class="bi bi-layout-text-window-reverse"></i>
...
<main id="main" class="main" style="box-shadow: 0px 0px 20px rgba(255, 255, 255, 0.1); background-color: #fff; border: 2px solid #B1D5E2;">
    <div class=" " style="margin-bottom:40px;">
        <h1 class="" style="font-family: Century Gothic; text-align: center;margin-bottom: 40px; color: #568894;">Liste des Randonneurs</h1>
       ...
       if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            // Affichage des données dans chaque ligne du tableau
            echo "<tr>";
            echo "<td>" . $row["CIN"] . "</td>";
            echo "<td>" . $row["Nom"] . "</td>";
            ...
            echo "<td><a href='supprimer_randonneuradmin.php?cin=". $row["CIN"]. "'>Supprimer</a></td>";
            echo "</tr>";
        }
    } else {
        echo "<tr><td colspan='7'>Aucun randonneur trouvé.</td></tr>";
    }
        ?>
    </table>
