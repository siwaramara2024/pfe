...
if (isset($_GET['query'])) {
    ...
    $sql = "SELECT * FROM Randonnee WHERE ID_randonnee LIKE '%$search_query%'OR libelleR = '$search_query' OR prix = '$search_query'
  OR libelleC = '$search_query'OR libelleA = '$search_query' OR libelleD = '$search_query' OR ID_agence = '$search_query' OR libelleV = '$search_query'";
}
$result = $conn->query($sql);

// Vérifier s'il y a une nouvelle inscription de randonneur
...
// Vérifier s'il y a une nouvelle agence de voyage
...
<hr class="dropdown-divider">
  ...
  <li class="nav-item">
    ...
    <li class="nav-item">
      <a class="nav-link "  href="liste_randonneeadmin.php">
        <i class="bi bi-layout-text-window-reverse"></i>
...
<main id="main" class="main" style="box-shadow: 0px 0px 20px rgba(255, 255, 255, 0.1); background-color: #fff; border: 2px solid #B1D5E2;">
    <div class=" " style="margin-bottom:40px;">
        <h1 class="" style="font-family: Century Gothic; text-align: center;margin-bottom: 40px; color: #568894;">Liste des Randonnées</h1>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . $row["LibelleD"] . "</td>";
        echo "<td>" . $row["LibelleC"] . "</td>";
        echo "<td>" . $row["LibelleA"] . "</td>";
        ...
        echo "<td><a href='supprimer_randonneeadmin.php?id=" . $row['ID_randonnee'] . "'>Supprimer</a></td>";
        echo "</tr>";
    }
} else {
    echo "<tr><td colspan='11'>Aucune randonnée trouvée.</td></tr>";
}
?>
    </table>
